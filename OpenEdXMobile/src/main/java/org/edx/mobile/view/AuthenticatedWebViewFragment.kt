...
open class AuthenticatedWebViewFragment : BaseFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (!isSystemUpdatingWebView) {
            ...
            val ivContentError: IconImageView = view.findViewById(R.id.content_error_icon)
            ivContentError.setIcon(FontAwesomeIcons.fa_exclamation_circle)
            contentErrorBinding?.contentErrorText?.text = getString(R.string.error_unknown)
            contentErrorBinding?.contentErrorAction?.visibility = View.VISIBLE
            contentErrorBinding?.contentErrorAction?.setText(R.string.lbl_reload)
            contentErrorBinding?.contentErrorAction?.setOnClickListener {
                UiUtil.restartFragment(
    ...
    companion object {
        ...
        fun newInstance(url: String, javascript: String?, isManuallyReloadable: Boolean): Fragment {
            val fragment: Fragment = AuthenticatedWebViewFragment()
            fragment.arguments =
                makeArguments(url, javascript, isManuallyReloadable)
            return fragment
        }
    }
}